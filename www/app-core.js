...
var BUILTIN_CHANGELOG = {
    ...
    'v1.2.1.8': '【新增】\n- **「网盘信息」按钮** —— 绑定后，「解绑」旁多了个一样大的小按钮；点开能看当前网盘的类型、服务器地址、账号\n- **密码脱敏显示** —— 应用密码只显示 ••••••••（已加密保存），绝不明文\n- **同步状态四维** —— 网盘数据 + 图片 / 徒步计划 + 徒步记录，一眼看清哪些数据同步了\n\n【优化】\n- **删掉「立即同步」按钮** —— 它和上方「上传」重复，避免按钮堆太多\n- **换机更省事** —— 云端上传 / 下载恢复都带上网盘配置（含加密后的密码）：换手机、重装后从云端恢复，账号密码一键配好\n- **导入本地备份同理** —— 恢复时按你的选择一键配好\n\n【内部】\n- 数据管理说明、隐私政策同步更新（密码加密存储、备份文件里含加密配置的说明）',
    'v1.2.1.7': '【修复】\n- **更新弹窗里的日志只显示一半** —— 以前被截断在前 400 字，现已显示完整，内容区可上下滚动\n- **去掉重复标题** —— 弹窗里那行「## v1.x.x 更新内容」已删除，与设置页「更新日志」观感一致\n\n【优化】\n- **弹窗宽度全面锁定** —— 以前部分弹窗只写了「最大宽度」没写实际宽度，浏览器会按内容自适应、在不同字体和设备下宽度漂移（「绑定账号」弹窗那次跳变就是这类）；现已把 10 处弹窗全部锁定，观感更整齐\n\n【内部】\n- 一轮例行体检：设计四维（圆角 / 字体 / 玻璃 / 层级）全绿、代码零死函数、滚动与动画均已节流、清理 7 张临时截图、全套自检全绿',
    'v1.2.1.6': '【修复】\n- **绑定账号弹窗会变窄** —— 选「其他 WebDAV」时宽度从 340px 缩到 276px（弹窗外壳按「内容自适应」定宽），现已固定，两个网盘选项下一样宽\n- **另外两个弹窗同毛病** —— 「发现网盘配置」「发现新版本」内容一变宽度就变，一并锁定，三个弹窗现在同宽同款\n- **切到「其他 WebDAV」后账号框残留** —— 还留着坚果云邮箱、提示文字也还写「你的坚果云邮箱」；现在提示与内容都跟着网盘类型走，你自己敲进去的账号不会被清掉，切回坚果云会自动恢复\n- **密码提示写死「16 位」** —— 其他 WebDAV 服务商的密码长度并不都是 16 位，现统一为「应用密码，不是登录密码」\n- **切后台再回前台偶尔弹「出错了：未知错误」** —— 根因：这类报错来自系统底层注入的脚本，浏览器对它们不给错误对象，旧代码只读那个对象，于是把真正的原因丢掉了。现在有原因的会显示真原因；完全没信息的只记进诊断日志，不再弹窗打扰\n\n【优化】\n- **坚果云副标题精简** —— 「推荐 · 免费额度够用」→「推荐」\n- **其他 WebDAV 副标题精简** —— 「自建 / 群晖 / 其他网盘」→「自建 / 其他网盘」\n\n【内部】\n- 系统底层的 WebDAV 回调注入加固（调用前判断回调存在 + 异常兜住 + 回调名做格式校验）—— 从源头不再产生「无错误对象的脚本报错」\n- 全局错误处理新增「静默记录」：无信息量的错误只进诊断日志，不再打扰（有信息量的照旧弹窗）\n- 回归测试 90 → 101 条（新增：弹窗两态等宽、占位随网盘切换、残留账号清理、无信息错误不弹窗等 11 条）',
    'v1.2.1.5': '【修复】\n- **「解绑账号」的确认框** —— 以前是系统自带的（灰底方块 + 系统字体，和 App 完全两个样子），已换成 App 同款玻璃弹窗：标题、说明、灰蓝「取消」+ 红框「解绑」\n- **「删除云端备份」的确认框** —— 同样是系统框，已一并换成玻璃弹窗\n\n【优化】\n- **普通选项不再用红色** —— 红色是「重要 / 危险操作」专用色（删除、解绑这类）；导出 / 导入弹窗里的选项现在改为中性玻璃按钮 + 清晰描边，一眼看得出边界\n- **勾选框改成 App 自绘** —— 靛蓝玻璃勾 + 圆角，不再是浏览器原生方框（iOS 上以前是系统样式，和界面不搭）\n- **玻璃按钮补「手型」光标** —— 全 App 统一\n\n【内部】\n- 新增统一的确认弹窗组件：即使弹窗被意外关掉，流程也能正常继续，不会卡住\n- 全项目扫查确认：已无任何系统原生弹窗（confirm / alert）残留\n- 更新日志从此按「新增 / 优化 / 修复 / 内部」分组逐条写全，不再漏项',
    'v1.2.1.4': '【新增】\n- **设置页「数据同步」换新样子** —— 以前是折叠区 + 三个输入框（服务器地址 / 账号 / 密码），现在是一张账号卡：没绑定时一句话说明 + 一个「绑定账号」按钮；绑定后显示用的哪个网盘、账号和上次同步时间\n- **绑定分两步走** —— 「选网盘」→「填账号」；选坚果云会自动填好地址，也可选「其他 WebDAV」自己填\n- **备份可以带上网盘配置** —— 导出时默认勾「同时包含网盘配置」（只含地址和账号，不含密码）；换新手机导入后地址和账号自动填好，补一次密码就能同步\n- **想彻底免填也行** —— 可勾「连应用密码一起带上」（默认不勾，勾之前有明确提醒：这份文件一旦外泄，别人就能访问你的网盘）\n- **导入带配置的备份会先问一句** —— 「只恢复记录」/「恢复配置」两个按钮并排，不会偷偷改你本机的账号\n\n【优化】\n- **导出 / 导入弹窗统一外壳** —— 遮罩浓度、层级、iPhone 底部安全区全部与全站一致（此前这两个弹窗是单独拼的样式）\n- **按钮统一玻璃款** —— 此前是另一套白玻璃配方，边缘和背景糊在一起，分不清按钮边界\n- **补「手型」光标** —— 全 App 玻璃按钮（此前鼠标移上去还是箭头，不像按钮）\n- **清掉废弃样式** —— 设置页那条已废弃的「配置服务器 / 账号」折叠条彻底移除\n\n【修复】\n- **深色遮罩恢复正常浓度** —— 导出 / 导入弹窗此前偏淡，和别的弹窗不一致\n- **圆角补齐** —— 个别按钮此前塌成直角，与全站 12px 圆角不一致\n- **同步状态弹窗文字贴按钮** —— 中间没有间距，现已留出正常间距\n- **备份里的密码不再「越界」** —— 手动导出时勾过「带密码」，不会再影响之后自动上传的备份（此前这个设置会留在内存里）\n\n【内部】\n- 清理 24 条已废弃的样式规则 + 4 处失效的类名引用（对应换掉的旧配置入口与旧按钮）\n- 弹窗体系首次纳入回归测试（容器层级 / 遮罩浓度 / 旧写法残留 / 按钮配色一致性，本轮 +8 项断言）',
    'v1.2.1.3': '【修复】\n- **数据更稳妥** —— 从外部导入备份时，个别记录字段格式稍有出入（比如海拔、难度被存成了文字）以前可能被静默丢弃，现在会自动修正后保留下来\n- **搜索更可靠** —— 个别记录字段格式不标准时，搜索不会再整体失效\n- **日期异常不再显示 NaN** —— 记录日期异常时，列表里不会再出现「NaN」字样（年份分组也一并修好）',
    'v1.2.1.2': '【修复】\n- **「天气 / 心情」弹窗按钮贴在一起** —— 「清空」和下面的「取消 / 确定」在真机上看着像重合，现在中间留出正常间距\n- **同族弹窗一并统一** —— 「难度」「日期时间」「年月（热力图）」的内容与按钮之间同样拉开了间距，看着更整齐',
    'v1.2.1.1': '【修复】\n- **概览渐入动画失效** —— 从别的页面切回概览时，统计卡的渐入动画不见了（点一下当前页面再切回来，更容易遇到），现已恢复正常\n- **顺带修好统计卡抬起效果** —— 鼠标移到统计卡上那点轻微抬起，之前被同一处问题压住了，一起恢复',
    'v1.2.1.0': '【优化】\n- **更省电** —— 切到后台（锁屏 / 切去别的 App）时，背景的流光和图标呼吸动画会自动暂停，回到前台继续，动效一个没少\n- **云端提示更稳妥** —— 云端返回的文字和备份文件名都会先做安全处理，不会再被奇怪字符撑乱界面\n\n【修复】\n- **设置页卡片与开关玻璃质感统一** —— 之前透明浓度、模糊程度和别处不一样，放一起看有差别',
    'v1.2.0.10': '【修复】\n- 「我的里程碑」里有些文字说明莫名分成两行（比如「累计 1000 公里」下面那句）——原因是右上角的进度数字把文字挤窄了，现在都回到一行\n- 顺带：五岳、陕西名山那类清单也不再被挤成两行\n\n【新增】\n- 首页的新人引导补了一条：顶栏标题可以自定义，改成你自己的名字\n\nMade by XiXi 💛',
    'v1.2.0.9': '【修复】\n- 天气收藏家收不到天气：记录里的天气是按图标存的，之前认不出来，现在能正常识别了\n- 成就现在会跟着记录变：删掉某座山，对应成就（比如陕西名山）会退回未达成，不再一直挂着"已达成"；重新达成会再庆祝一次\n- 「我的里程碑」弹窗的滚动条跑到弹窗圆角外面了，已归位\n\n【优化】\n- 概览页的「我的里程碑」入口卡，也跟其它卡片一样渐入登场了\n\nMade by XiXi 💛',
    ...
    'v1.2.0.4': '【修复】\n- 计划页搜索框不再「一闪就没」：以前切到计划页，搜索框出现一下、还没点就自动收走了；现在它会一直留在那儿，随时都能用（日历视图、列表视图都一样）\n\nMade by XiXi 💛',
...
// ★2026-09-18 更新日志正文渲染：把纯文本解析成「分组标题 + 缩进子条目」的层级结构。
//   背景（用户 2026-09-18 反馈「字都堆在一起，不好看」）：旧实现把整段 body 用 white-space:pre-wrap
//   原样输出 → 【修复】与正文同字号同色、多条挤成一大段没有条目间距、每个版本末尾还各带一行
//   Made by XiXi（三版就重复三次）。
...
function renderChangelogBody(text, CL, MUTED) {
    ...
    for (i = 0; i < lines.length; i++) {
        ...
        if (/^#{1,6}\s/.test(t)) continue;                          // 历史的「## vX 更新内容」：弹窗标题已含版本号，丢弃
        if (/^Made by XiXi/i.test(t)) continue;                       // 每版都写的签名 → 丢弃，弹窗末尾统一显示一次
        mHead = t.match(/^【([^】]{1,14})】$/);
        ...
        html += '<div style="margin:0 0 7px;line-height:1.7;">' + inline(t) + '</div>';
    }
    if (!html) html = '<div style="line-height:1.7;">' + inline(String(text || '')) + '</div>';
    return '<div style="font-size:13px;line-height:1.7;color:' + CL.b + ';word-break:break-word;">' + html + '</div>';
}
...
function showChangelogModal() {
    ...
    modal.innerHTML = '<div class="confirm-modal-content modal-fade-scale" style="max-width: 360px;width:calc(100vw - 44px);box-sizing:border-box;">' +
    ...
    try {
        ...
        for (var j = 0; j < parts.length; j++) {
            var pt = parts[j];
            var rbBody = renderChangelogBody(pt.body, CL, MUTED);
            html += '<div style="display:flex;align-items:center;gap:8px;margin-bottom:10px;">' +
                ...
                rbBody +
                (j < parts.length - 1 ? '<div style="height:1px;background:rgba(148,163,184,0.25);margin:16px 0;"></div>' : '');
        }
        // ★2026-09-18 签名固定显示一次（以前每个版本末尾各带一行，三版就重复三次）
        html += '<div style="margin-top:16px;text-align:center;font-size:12px;color:' + CL.tagTx + ';">Made by XiXi 💛</div>';
...
// ★2026-09-17 修复「切后台再回前台弹『出错了：未知错误』」（用户反馈）：
//   根因：报错来自原生侧 evaluateJavascript 注入的脚本（WebDAV 回调等）。这类**无来源脚本**
//   一旦抛错，Chromium 不提供错误对象 ⇒ event.error 为 null；而旧代码只读 event.error，
//   于是真实原因（在 event.message 里，如 "Uncaught SyntaxError: ..."）被丢掉，只剩「未知错误」。
//   处理：① 有可读信息就照常提示（优先错误对象，其次 event.message）；
//         ② 彻底没信息（Script error. / 空 message / Promise 拒绝不带原因）→ 只写诊断日志、不弹窗。
//   「事后发现」类无信息提示不该打断用户；诊断日志仍可在 设置 → 关于应用 → 导出诊断 里看到。
function logSilentAppError(kind, extra) {
    try {
        console.error('[静默] ' + kind + (extra ? '（' + extra + '）' : '') + ' —— 无可读错误信息，已记录未提示');
    } catch (e) { /* 记录失败不影响业务 */ }
}
...
window.addEventListener('error', function(event) {
    ...
    var raw = event.error || event.message || '';
    var shown = extractErrMsg(raw);
    var noInfo = !raw || shown === '未知错误' || /^script error\.?$/i.test(String(raw).trim());
    if (noInfo) {
        logSilentAppError('无来源脚本错误' + (event.filename ? ' @' + String(event.filename).slice(-48) + ':' + (event.lineno || '?') : ''));
        return false;
    }
    notifyGlobalError('出错了：' + shown);
    return false;
});
...
window.addEventListener('unhandledrejection', function(event) {
    ...
    // ★同上：reason 为 undefined/null/空串时旧代码显示「未知错误」（零信息）→ 改为只记诊断日志
    if (event.reason === undefined || event.reason === null || event.reason === '') {
        logSilentAppError('Promise 拒绝未携带原因');
