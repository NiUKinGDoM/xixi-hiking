...
var BUILTIN_CHANGELOG = {
    ...
    'v1.2.1.5': '【修复】\n- 「解绑账号」的确认框以前是系统自带的（灰底方块 + 系统字体，和 App 完全两个样子）—— 已换成 App 同款玻璃弹窗：标题、说明、灰蓝「取消」+ 红框「解绑」\n- 「删除云端备份」的确认框同理（同样是系统框），已一并换成玻璃弹窗\n\n【优化】\n- 导出 / 导入弹窗里的几个选项按钮不再用红色 —— 红色是「重要 / 危险操作」专用色（删除、解绑这类），普通选项现改为中性玻璃按钮 + 清晰描边，一眼看得出边界\n- 导出弹窗里的勾选框改成 App 自绘样式（靛蓝玻璃勾 + 圆角），不再是浏览器原生的方框（iOS 上以前会显示成系统样式，和界面不搭）\n- 全 App 玻璃按钮补上「手型」光标\n\n【内部】\n- 新增统一的确认弹窗组件：即使弹窗被意外关掉，流程也能正常继续，不会卡住\n- 全项目扫查确认：已无任何系统原生弹窗（confirm / alert）残留\n- 更新日志从此按「新增 / 优化 / 修复 / 内部」分组逐条写全，不再漏项\n\nMade by XiXi 💛',
    'v1.2.1.4': '【新增】\n- 设置页「数据同步」换了新样子：以前要展开一条折叠区、填三个输入框（服务器地址 / 账号 / 密码），现在是一张账号卡 —— 没绑定时就一句话说明 + 一个「绑定账号」按钮；绑定后显示用的哪个网盘、账号和上次同步时间\n- 绑定弹窗分两步：「选网盘」→「填账号」。选坚果云地址会自动填好（不用再手输那串地址），也可以选「其他 WebDAV」自己填\n- 备份可以带上网盘配置了：导出时默认勾「同时包含网盘配置」（只含地址和账号，不含密码）。换新手机导入这份备份，网盘地址和账号会自动填好，只要补一次密码就能同步\n- 想彻底免填也行：导出时可勾「连应用密码一起带上」（默认不勾，勾之前有明确提醒 —— 这份文件一旦外泄，别人就能访问你的网盘）\n- 导入带配置的备份时会先问一句：「只恢复记录」/「恢复配置」两个按钮并排，不会偷偷改你本机的账号\n\n【优化】\n- 导出 / 导入弹窗改用与全站完全同一套弹窗外壳：遮罩浓度、层级、iPhone 底部安全区全部统一（此前这两个弹窗是单独拼的样式）\n- 导出 / 导入弹窗的按钮改用全站统一的玻璃按钮（此前是另一套白玻璃配方，边缘和背景糊在一起，分不清按钮边界）\n- 全 App 玻璃按钮补上「手型」光标（此前鼠标移上去还是箭头，不像按钮）\n- 设置页那条已废弃的「配置服务器 / 账号」折叠条样式彻底清除\n\n【修复】\n- 深色模式下，导出 / 导入弹窗的遮罩恢复正常浓度（此前偏淡，和别的弹窗不一致）\n- 弹窗按钮的圆角补齐（个别按钮此前塌成直角，与全站 12px 圆角不一致）\n- 「同步状态」弹窗里的文字和下面按钮贴在一起（中间没有间距）→ 现已留出正常间距\n- 备份里的密码不会「越界」：手动导出时勾过「带密码」，不会再影响之后自动上传的备份（此前这个设置会留在内存里，让自动备份也带上密码）\n\n【内部】\n- 清理 24 条已废弃的样式规则 + 4 处失效的类名引用（对应换掉的旧配置入口与旧按钮）\n- 弹窗体系首次纳入回归测试：容器层级 / 遮罩浓度 / 旧写法残留 / 按钮配色一致性（本轮 +8 项断言）\n\nMade by XiXi 💛',
