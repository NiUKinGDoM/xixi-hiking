...
var BUILTIN_CHANGELOG = {
    'v1.2.2.6': '【修复】\n- **记录里的数值不会再被填错** —— 距离填负数以前会被原样存下来（海拔和用时早就有保护，只有距离漏了这道），一路污染总里程、统计、分享卡和年度回顾；现在负数按 0 计，并加了 10000 公里的上限；用时也补上了上限（小时框 0~23、总时长不超过 1440 分钟），难度统一限定在 1~5 级（此前只有计划那边有这道限制，同一个字段两处口径不一样）。导入的旧备份里若有异常数值，载入时会被自动纠正\n- **保存失败现在会明确告诉你** —— 以前本机存储写不进去时，App 想提示却提不出来（提示语是写好的，但下层把错误吃掉了），你以为存好了，其实数据已经丢了。现在存储写入失败会立刻弹出「数据保存失败（本机存储异常），请检查可用空间后重试」，徒步记录与计划两处都改了\n- **本地数据读取异常不再「凭空消失」** —— 万一存储损坏，以前会显示成空列表，你一保存就把原数据永久覆盖了。现在会把出问题的原始数据单独备份保留，并在启动时提示「本地数据读取异常：原始数据已备份保留，未丢失」\n- **导入损坏的备份文件会有提示** —— 以前选到损坏的备份（压缩包不完整、包里的数据文件坏了），点了「导入」毫无反应，只能反复点；现在会明确告诉你「备份文件已损坏，无法导入（可换一个备份文件重试）」\n- **「抹掉足迹」更彻底了** —— 以前只清了记录和照片：各页的新手引导标记、里程碑、年度回顾标记、崩溃记录都还留着（再点进去会觉得「没清干净」），网盘绑定的账号密码也还留在设备上。现在一并清除；确认弹窗里也写明了「会解除网盘绑定，云端备份文件不受影响」\n\n【内部】\n- 记录字段的统一净化层补上距离与用时（此前只管难度和海拔，负数距离能一路进存储）\n- 删掉一个全项目没人引用的死属性（`data-diff`）\n- `todayKey` 去掉一次多余的日期字符串往返（结论相同，但易被误读成时区问题）\n- 新增 15 条守卫锁住上面这些修复，并逐条做了「撤掉修复必须报红」的反向验证\n- 全量自检 795 → **810 项全绿**（test.js 272 → 287）\n\nMade by XiXi 💛',
    'v1.2.2.5': '【新增】\n- **列表视图的到期计划也能「完成 / 延期」了** —— 过期或当天的计划，右侧那个对号按钮换成**日历循环图标**，点它会弹出「完成 / 延期」：选「完成」转入徒步记录并弹庆祝卡，选「延期」直接打开编辑弹窗改日期；未到期的计划仍是那个对号，行为不变。以前列表视图只有「完成」一个选择，同一个计划切到日历视图行为却不一样\n\n【优化】\n- 深色模式下弹窗标题的蓝色小图标看得清了 —— 以前那抹靛蓝在深色弹窗上对比度只有 2.64:1，看着发闷、几乎和背景融在一起；现在换成浅靛（8.33:1），一眼能看清（隐私政策、完成/延期、确认完成、删除确认等所有弹窗的标题图标都受益）\n\n【内部】\n- 消除一处重复代码：计划日差算法原先在 4 个地方各写了一遍（到期判定、计划徽章、过期关怀弹窗、计划提醒通知）—— 现统一走一个公共函数 `planDayDiff()`，以后改口径只改一处，不会再出现「徽章说过期、按钮说没过期」这类不一致\n- 性能实测：概览/记录/计划/设置四个页签全部 60fps，滚动 60.1fps，无卡顿帧、无长任务；顺带让列表每条计划渲染少构造 4 个日期对象\n- 设计体检：把「勾选框 5px 圆角」正式登记进项目设计规范表（此前是整个项目唯一一处未登记的取值），体检恢复全绿\n- 视觉回归：概览页截图容差由 0.5% 放宽到 1% —— 该页玻璃卡密集且带入场动画，动画终态存在 0.1%~0.6% 的亚像素抖动（其余 9 张截图仍按 0.5% 严格比对）\n- 修正 3 处注释里的错字（「靶蓝」→「靛蓝」）\n- 全量自检 792 → **794 项全绿**（test.js 271、P0P3 299、E2E 126）\n\nMade by XiXi 💛',
    'v1.2.2.4': '【新增】\n- 计划到期了可以「完成 / 延期」 —— 日历视图里，**过期或当天**的计划，右侧按钮从「完成」变成「完成/延期」：点开先问一句，已经走完就点「完成」，会转入徒步记录并弹出庆祝卡（可以继续补心情、天气、照片）；还没走就点「延期」，直接打开编辑弹窗改个新日期。明天及以后的计划保持原来的「完成」按钮（以前只有「完成」一个选择：日子到了但没去走，只能先完成后删除，或者自己进编辑里改日期）\n\n【修复】\n- 打开应用偶尔弹「已自动上报一次崩溃报告」 —— 已改成静默。以前只要应用出过错、而且配了网盘，下次启动就会自动上报一次并在屏幕上弹这句提示；它属于「事后通知」，不该打断你正在做的事。上报记录仍然保留在「设置 → 关于应用 → 导出诊断」里，需要排查时能看到错误内容和时间\n\n【内部】\n- 到期判定抽成公共函数，与「已过期 N 天 / 今天 / 明天」徽章**同一口径**（按本机零点算日差，≤ 0 视为到期）—— 避免出现「标着已过期 3 天、按钮却还是普通完成」这种不一致\n- 日历里**单日明细**和**整月明细**两处同时接入（口径一致）；列表视图按要求保持原样\n- 「完成/延期」弹窗宽度显式锁定（只写 `max-width` 会被外层容器挤窄，实测只有 280px）；弹窗宽度守卫自动纳入，37 → **38 个容器**\n- 新增回归守卫 26 条：源码级 10 条（判定口径 / 双处接入 / 「完成」走既有流程 / 「延期」打开编辑弹窗 / 未过期不受影响 / 宽度显式锁…）、运行时 12 条（判定边界 4 条 + 渲染出的按钮文案 + 弹窗三按钮 + 延期与完成两条链路）、真实浏览器 3 条（三态按钮文案 / 弹窗三按钮 + 延期 → 编辑弹窗 / 完成 → 记录 +1 + 自动切记录页）\n- 反向验证：临时撤掉修复跑一遍，确认守卫真的会报红（运行时 3 条 + 源码级 1 条），随即还原\n- 顺手填两个工具链坑：① 自造数据的 E2E 用例必须收尾清理残留弹层（庆祝卡、计划编辑弹窗都不在 `closeOpenModals` 管辖内，漏一个就会挡住后续点击 → E2E 以退出码 2 异常退出，排查时极易误读成断言失败）② 静默日志不能用 `console.log`（审计会判为调试残留，而 checkall 不含审计，会在发版时才暴露）→ 统一改 `console.error`\n- 全量自检 756 → **782 项全绿**（test.js 256 → **266**、P0P3 282 → **294**、E2E 121 → **124**）\n\nMade by XiXi 💛',
    'v1.2.2.3': '【优化】\n- 浅色模式下弹窗里的文字整体加深 —— 隐私政策、免责声明、更新日志、网盘信息、记录/计划详情里的标题、正文、说明与标签都提了一档（标题 #0f172a、正文 #1e293b、次要文字与标签 #334155）。以前这些字是 #52606f 中灰（在弹窗上只有 3.25:1），更新日志条目更淡到约 1.9:1，叠在玻璃弹窗的中灰底上看着费眼；现在都在 5.2:1 ~ 9.0:1\n- 计划页引导卡补上「还能切换列表」 —— 改为「把下一个山头定好日子：日历看排期、一眼看清哪天有行程，右上角可切换 列表 看全部计划。到点会提醒你出发，完成时一键补成正式记录。」（以前只提日历，新用户不知道右上角还能切成列表看全部计划）\n\n【修复】\n- 更新日志条目文字过淡 —— 原本是 rgba(100,116,139,0.72)（约 1.9:1，几乎糊在背景里）→ #334155（5.2:1）\n- 政策里的蓝色链接、更新日志顶部的「本次」标签、绑定网盘步骤标签颜色偏浅 —— #4f46e5 仅 3.17:1、#4338ca 仅 3.98:1 → 统一改深靛蓝 #3730a3（5.0:1）\n- 浅色下网盘信息标签、照片占用「共 N 张」、记录/计划详情的小标签与关闭按钮 —— #52606f（3.25:1）/ #64748b（2.40:1）→ #334155（5.2:1）\n\n【内部】\n- 对比度是「量出来的」而不是猜的：截图采像素测得浅色玻璃弹窗的实际合成底色约 #B8B8B8（深遮罩 + 半透明卡片叠出来的中灰），再按这个底色逐个算色值 —— 全部 17 个弹窗、302 个文字元素扫了一遍\n- 深色模式不在本次范围（原本对比度已达标），只改浅色分支；页面上（非弹窗）的小灰字**有意保持原样** —— 它们的底色是浅色页面（约 #F5F7FA），同一色值实测 6.3:1 已达标，改深反而让页面显重\n- 新增源码级对比度守卫 8 条（按实测底色 #B8B8B8 计算，正文字号低于 4.5:1 直接报红），防止以后又把文字调浅；并修正 1 条因本次加深而过时的旧断言\n- 新增引导卡断言 1 条（锁住「右上角可切换 列表」这句文案）；1 条徽章颜色断言随本次加深同步更新\n- 视觉基线刷新 3 张（隐私政策 6.15%、免责声明、同意留存 2.61%）—— 差异区域分析确认「变深像素 20041 / 变浅 212、差异带全是等距文字行」属纯文字加深，无结构变化\n- 全量自检 746 → **756 项全绿**（test.js 248 → **256**、P0P3 281 → **282**）\n\nMade by XiXi 💛',
...
function changelogPalette() {
    ...
    return {
        CL: {
            ...
            tagTx: IS_DARK ? '#a5b4fc' : '#3730a3',   // 2026-09-18 对比度实测：11px 标签 #4f46e5 仅 3.17:1 → 深靛蓝 #3730a3（4.5:1）
            tagBg: IS_DARK ? 'rgba(99,102,241,0.22)' : 'rgba(99,102,241,0.12)',
            tagBd: IS_DARK ? 'rgba(129,140,248,0.55)' : 'rgba(99,102,241,0.35)'
        },
        // 2026-09-18 对比度实测：浅色弹窗合成底色≈#B8B8B8，原 rgba(100,116,139,0.72) 仅约 1.9:1
        MUTED: IS_DARK ? 'rgba(148,163,184,0.85)' : '#334155'
    };
}
...
var AppStore = {
    setItem: function (key, value) {
        // ★2026-09-20 返回值修复：原先内部 try/catch 吞掉异常 → 上层 saveToStorage 的「数据保存失败」提示永远不触发（静默丢数据）
        try { localStorage.setItem(key, JSON.stringify(value)); return true; } catch (e) { console.warn('[Store] setItem failed:', key, e && e.message); return false; }
    },
    getItem: function (key) {
        ...
        try {
            ...
            // ★2026-09-20 解析失败的原始串先隔离保存：此前直接返回 null → 用户看到「数据全没了」，
            //   一旦再保存就把损坏内容永久覆盖。隔离一份，至少还能人工捞回。
            try { if (raw) localStorage.setItem(key + '_corrupt', raw); } catch (e2) { /* 配额满时放弃隔离 */ }
            console.warn('[Store] getItem failed:', key, e && e.message);
            return null;
        }
    },
    ...
    photoGetAll().catch(function () { return []; })
        .then(function (res) {
            ...
            var P = {
                ...
                sub: IS_DARK ? '#8b9ab0' : '#334155',   // 2026-09-18 对比度实测：#64748b 在弹窗合成底≈#B8B8B8 上仅 2.40:1 → #334155（5.2:1）
