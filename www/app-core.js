...
var BUILTIN_CHANGELOG = {
    'v1.2.2.10': '【修复】\n- **记录页列表的删除按钮和计划页统一成灰色垃圾桶图标** —— 以前记录页列表里是红色垃圾桶，和计划页的灰色垃圾桶不是一个样；现在两处完全一致，翻列表时更整齐\n- **计划页日历明细的删除按钮补上了边框** —— 以前这个删除按钮没有边框、孤零零一个字；现在有了和列表里一样的边框，看起来更完整\n- **足迹热力图弹窗里的「分享」按钮文字能看清了** —— 以前在浅色背景下这个按钮的文字是白色的、几乎看不见；现在文字会跟着深色/浅色模式变化，两种模式都清楚\n\nMade by XiXi 💛',
    ...
    'v1.2.2.8': '【修复】\n- **「完成」「编辑」「保存」「确定」「分享」这些按钮不再是红色** —— 以前它们和「删除」用的是同一种红，一眼看不出哪个动作有风险；现在它们回到中性玻璃色，红色只留给会丢数据的操作（删除记录、批量删除、抹掉足迹、清理无用照片）\n\n【优化】\n- **列表里的删除按钮改成一枚淡红小图标** —— 以前整列都是红框按钮，一屏几十个红，红色反而失去了警示作用；现在列表一眼扫过去是干净的，需要删的时候又找得到\n- **《隐私政策》补充了联系邮箱** —— 反馈问题可以直接发邮件到 Xixihiking@foxmail.com，不必绕到 GitHub；因为政策正文有更新，打开 App 时会再让你确认一次条款，同意后不再打扰\n\nMade by XiXi 💛',
    'v1.2.2.7': '【修复】\n- **删除类操作现在才是红色** —— 以前「保存」「编辑这条记录」「绑定账号」「支持作者」这些普通按钮和「删除」用的是同一种红色，一眼看不出哪个动作有风险；现在普通按钮改成中性玻璃色，红色只留给会丢数据的操作（删除记录、批量删除、抹掉足迹、清理无用照片）\n- **小屏手机上页面标题不再被挤成竖排** —— 以前屏幕窄的时候，右侧「批量 / ＋ / 山册」三个按钮会把「记录」「计划」这类标题挤成两行竖排；现在标题始终横排，位置不够时按钮自动换到标题下方一行\n\n【优化】\n- **编辑记录的时间显示更清爽** —— 以前是「2026-01-05T08:30」这种带 T 的生硬写法，现在是「2026-01-05 08:30」；点开仍是原来的日历时间选择器，保存下来的时间也不会变\n- **编辑弹窗的输入框统一成玻璃质感** —— 输入框、时间框和「时 / 分 · 里程」框以前是不透光的白方块，跟弹窗的玻璃风格不像一家人；现在统一成半透明玻璃，空白字段的提示字（如「输入名称」「同行人」）也比以前更清楚好读\n\nMade by XiXi 💛',
...
function openPhotoUsageDetailModal() {
    ...
    modal.innerHTML = '<div class="confirm-modal-content modal-fade-scale" id="puModal" style="max-width:392px;width:calc(100vw - 44px);box-sizing:border-box;padding:18px 16px 14px;">' +
        ...
        '<button class="glass-btn ripple-effect" type="button" id="puOpt" style="flex:1;padding:7px 0;border-radius:10px;font-size:13px;font-weight:600;margin-left:8px;">优化</button></div></div>';
    document.body.appendChild(modal);
    modal.addEventListener('click', function (e) { if (e.target === modal) document.body.removeChild(modal); });
