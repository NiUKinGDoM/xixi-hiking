...
var BUILTIN_CHANGELOG = {
    'v1.1.8.4': '## v1.1.8.4 更新内容\n\n**年度回顾**\n- 概览页「统计概览」旁新增「年度回顾」入口，点开是全屏回顾页：次数/里程/用时/山峰数/平均里程 + 12 个月徒步节奏柱状图 + 年度之最（最远单程/最长时间/最高海拔/最常去/最多同行/第一次/最后一次出发）\n- 每年 1 月 1 日打开 App 自动弹出上一年的回顾\n- 最常去/最多同行只在真的有反复去过/反复同行的才显示\n\n**我的山册**\n- 记录页新增「山册」视图（与列表一键切换）：每座山一张卡，去过次数/总里程/总用时/最高海拔/平均难度/一起走过的人，点开看这座山每一次记录，点 ↗ 直达那条记录\n- 山册支持搜索（和列表同一个搜索框，切视图自动切换提示）\n\n**计划提醒完善**\n- 计划过期后独立提醒「还没去」（只提醒最近 3 天过期的，不轰炸，同一天不重复）\n\nMade by XiXi 💛',
