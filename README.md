...
- 📅 **计划**：计划徒步管理、难度选择、完成标记、**过期关怀**（过期计划列表标「已过期 N 天」，启动弹一次可顺延一周/去处理）、**列表/日历双视图**（默认日历，切换按钮固定最右）、**日历整月明细**（每条标日期，点日期聚焦 + 「整月」按钮，二次点击 tab 回当天）、列表按年份分组、**完成 → 自动进入记录编辑补全**（预填名字/难度/海拔）+ **庆祝卡片**（400 粒彩屑）、**启动提醒 + 每天 08:00 系统通知栏**（不打开 App 也能收到，**手机重启自动恢复**）
...
- ⚙️ **设置**：
  ...
  - 主题（跟随系统/浅色/深色）、**显示大小三档**（标准/大/特大，整体放大不费眼）、震动开关、**照片占用统计 + 孤立照片扫描清理**（P0-3）
...
- 🔄 **当前版本**：v1.1.9.7（vc227 · 2026-09-06）
...
├── test.js                  # 自动测试①（改完必跑：语法/关键函数/死代码/结构/版本/数据层/schema/照片GC/P1体验/新功能源码 89 项）
├── test-ui.js               # 自动测试②（jsdom UI 渲染测试 30 项：记录/计划渲染搜索分页徽标空态/键盘跟随/批量/弹窗防重入/转义）
└── _test_p0p3.js            # 自动测试③（jsdom 集成链路 140 项：…详情照片横滑/编辑布局顺序/批量勾选/三步引导/小日记/全文搜索/字号/抹掉数据/过期关怀/回忆册打印等，发布前必跑）
```
