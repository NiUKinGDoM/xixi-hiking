...
| 分享 | 🖼️ 一键生成分享卡（海拔/难度/心情/天气/同行人） |
| 设置 | ⚙️ 完整备份 zip / 纯数据备份 / WebDAV 坚果云同步（自动保留 2 份）/ 导出诊断 / 检查更新（内置离线更新日志）/ 深色模式 / 震动开关 / 照片占用优化 / 抹掉足迹 |
| 安全 | 🛡️ 安装包签名自校验（防重打包）+ 打包代码混淆 + 资源完整性校验 + 系统备份关闭（v1.1.10.8 起） |
