...
- 📊 **概览**：统计卡片（总记录数/平均海拔/最高海拔/平均难度）、难度分布柱状图（独立卡片）、**年度足迹热力图** + 累计爬升
- 📝 **记录**：徒步记录增删改、排序、行内编辑、难度 1-5 级（玻璃徽章）、默认按时间排序、**心情/天气下拉文字选择**（存储 emoji，名称后仍显示天气表情）
- 📷 **照片**：每条记录最多 9 张（canvas 压缩 1280px/JPEG0.7，IndexedDB 存储）、**编辑行层叠卡片展示**（无照片时白框加号）、**灯箱查看/保存/翻页 + 编辑模式添加/删除**
- 📅 **计划**：计划徒步管理、难度选择、完成标记、**默认按计划时间由近到远排序**、**启动提醒 toast**（今天/计划未完成/明天·后天·大后天）
- 📳 **震动反馈**：点击按钮短震（设置可开关，默认开，强度跟随系统）
- 🖼️ **分享卡**：热力图详情一键生成（分享卡背景 + 白渐变遮罩 + 艺术字，含海拔/难度/心情/天气/同行人）
- 📊 **月度小结**：概览「本月 X 次 · 累计爬升 Y 米」
- ⚙️ **设置**：
  - **完整备份**（HTML 单文件：记录 + 照片 + 计划，可导入恢复、浏览器可看）/ **纯数据备份**（不含照片）
  - **导出诊断报告**（版本/数据量/错误日志，问题排查用）
  - **WebDAV 数据同步**（坚果云）：上传/下载/管理云端备份、自动同步开关、**自动保留最近 2 份**、**密码加密存储**
  - **应用内检查更新**（GitHub Release + 国内镜像）：确认弹窗 → 检查 → 立即更新
...
- 🔄 **当前版本**：v1.1.3.5（vc159 · 2026-08-25）
